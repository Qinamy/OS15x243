...
tid_t
process_execute (const char *file_name) //file_name是字符串，已经被虚拟磁盘保存的文件
{
  char *fn_copy;
  tid_t tid;
  ...
  file_name = strtok_r (fn, " ", &save);//lib/cernal/string.c中定义的函数，把fn分割成空格前面的部分和空格后面的部分
  /* == My Implementation */
  ...
  /* My Implementation */
  tid = thread_create (fn, PRI_DEFAULT, start_process, fn_copy);//创建用户线程fn,函数start_process，fn_copy
  if (tid == TID_ERROR)
    goto done;
...
{
	/*实现Echo exit(0)为正常退出、Bubsort exit(-1)为异常退出的功能。*/
  /* Old Implementation
  return -1; */
