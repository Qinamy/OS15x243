...
static int
sys_close(int fd)//调用pintos已经写好的函数
{
  struct fd_elem *f;
  int ret;
  
  f = find_fd_elem_by_fd_in_process (fd);//将fd在链表中便利一遍，找到需要的fd，返回fd_elem结构体的f
  
  if (!f) /* Bad fd */
    goto done;
  file_close (f->file);//pintos内部自带的
  list_remove (&f->elem);
...
static int
sys_exec (const char *cmd)//调用prosess_execute函数
{
  int ret;
